...
2. `glimpse()` shows you all variables (columns) in the data, and a 'glimppse of the first bunch of elements)()

Other interactice things to point out: 'using 'tab' to bring up suggested columns, so you can filter through them, useful when you have very many variables, and you can't remember what you called them.
...
bat_dat %>% # drop the nspp per Mmacr col
        ...
        group_by(Habitat, Season) %>%
        gather(key = species, value = richness, -Habitat, -Site, -Season) %>% # illustrate what happens when we don't choose to ignore the other columns by using the 'minus' sign... they get included as an entity in the key column 'species' and it gets confused about what value to put in the 'richness' column for everything... 
...
### Site / Environmental covariate influence on bat richness: Continuous response and predictor variables... (so far been working with dichotomous predictor).

Let's pick a handful of individual species to investigate the effects of some continuous predictor variables on their richness...
