...
To alter an existing variable we use the name of that variable in the name-value pair argument to `mutate()`, and then provide some computation on the right as we did for creating a new variable with `mutate()`.

Imagine there was an error at some point during data collation and a decimal place was shifted, let's pretend it was for House500. 
