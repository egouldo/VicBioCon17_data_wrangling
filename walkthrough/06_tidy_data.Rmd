...
### Why it is often good to have a relational database or colection of dataframes rather than a single "tidy" dataframe

- a site versus observation dataframe - provide the example in this bat data of finding errors dont want to be transcribing or copying and pasting values when recording data EVER. This can happen when you have multiple observations per sampling unit that depend on what's present. OR in this case when you have repetitive sampling through time... multiple visits at a site.. the site variables dont change necessarily. So you have to enter them by hand twice.

NA's vs missing values: NAs should be written as `NA` truly missing data and 0.
